...
def _get_instance_id_win32():
    ...
    i = 0
    while True:
        name = 'Global\\NeuralCloudAutoHelper.%08X.%d' % (path_hash, i)
        import ctypes
...
def _get_instance_id_posix():
    ...
    while True:
        try:
            if i == 0:
                filename = os.path.join(logs, 'NeuralCloudAutoHelper.log')
            else:
                filename = os.path.join(logs, 'NeuralCloudAutoHelper.%d.log' % i)
            f = open(filename, 'a+b')
            f.seek(0)
            import fcntl
...
def get_instance_id():
    ...
    if _instanceid == 0:
        logfile = os.path.join(logs, 'NeuralCloudAutoHelper.log')
    else:
        logfile = os.path.join(logs, 'NeuralCloudAutoHelper.%d.log' % _instanceid)

    with open(logging_config_file, 'r', encoding='utf-8') as f:
        logging.config.dictConfig(yaml.load(f))
    logging.debug('NeuralCloudAutoHelper version %s', version)
    return _instanceid
