...
def _create_helper(use_status_line=True, show_toggle=False):
    from NeuralCloud.helper import NeuralCloudAutoHelper
    _ensure_device()
    frontend = ShellNextFrontend(use_status_line, show_toggle)
    helper = NeuralCloudAutoHelper(device_connector=device, frontend=frontend)
    if use_status_line:
...
global_cmds = [interactive, help]
interactive_cmds = [connect, auto, exit]
